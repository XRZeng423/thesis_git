...
def list_variables(data_dir, file_name):
	...
	list_dic = {'Identifiers' : ['permno', 'ticker', 'comnam', 'conm', 'gvkey', 'cusip', 'lpermco'],
				...
				'Fama-French' : ['mktrf', 'smb', 'hml', 'rf', 'umd'],
				'Fundamentals' : ['aco', 'act', 'ajex', 'am', 'ao', 'ap', 'at', 'capx', 'ceq', 'che', 'cogs', 
	...
	for item in list_dic.keys():
		...
		not_others = not_others + [x for x in header if x in list_dic[item]]
	print('Others:')
	print('%s' %([x for x in header if x not in not_others]))
...
def load_data(data_dir, file_name, variable_list = []):
	...
	## List of variables
	basic_list = ['permno', 'daret', 'retx', 'vol', 'shrout', 'prc', 'shrcd', 'exchcd', 'ticker', 'ldate', 'conm', 'me', 'be', 'revt', 'cogs', 'at', 'hml', 'smb', 'mktrf', 'rf']
	...
	df_full[['ldate', 'rf', 'mktrf', 'smb', 'hml']].drop_duplicates().to_stata('FFData.dta')

	## Columns to use
	# basic_list = ['permno', 'daret', 'retx', 'vol', 'shrout', 'prc', 'shrcd', 'exchcd', 'ticker', 'ldate', 'conm', 'me', 'be', 'revt', 'cogs', 'at', 'hml', 'smb', 'mktrf', 'rf']
	# aux_list = ['ldate_lag', 'ldate_lag12', 'screen', 'me_lagged', 'screen12']
	# final_list = list(set(basic_list + aux_list + variable_list))
	# df_full = df_full[final_list]
...
def rank(df, var_name):
	...
	return df.groupby('ldate')[var_name].rank(axis = 1, ascending = True)
...
def create_portfolios(df, sort_frequency, num_port):
	...
	for por_num in range(1, num_port + 1):
		...
		df_rets['retP_vw_P%d' %(por_num)] = sub_df.groupby('ldate')['wgtd_daret'].agg(sum)        
...
def analyze_strategy(df_strategy, analysis_type):
	...
	df_ff = pd.read_stata('FFData.dta').sort_values(['ldate']).drop(columns = ['index']).rename(columns = {'ldate' : 'ym'})
	...
	if analysis_type == 'Performance':
		...
		print('(4): Long-Short Value Weights ~ Fama French Model')
		print('(5): Long-Only Rank Weights ~ Fama French Model')
		print('(6): Long-Short Rank Weights ~ Fama French Model')
...
def construct_signal(data_dir, file_name, file_type, strategy_name, sample_start, sample_end, remove_micro_caps):
	...
	print('> Constructing Signals for Strategy: %s...' %(strategy_name))

	if strategy_name == 'Size':

		df_full['signal'] = 1 / df_full.groupby(['permno'])['me'].shift(1).multiply(df_full['screen'])
		
	elif strategy_name == 'Value':

		df_full['signal'] = df_full.groupby(['permno'])['be'].shift(1).multiply(df_full['screen'])/df_full.groupby(['permno'])['me'].shift(1).multiply(df_full['screen'])
		...
		# In month t, add the returns from t-12 to t-1. Hence, we skip one month
	...
	elif strategy_name == 'LowBeta':

		df_full['signal'] = -df_full.groupby(['permno'])['beta'].shift(1).multiply(df_full['screen'])

	elif strategy_name == 'Quality':

		# Signal 1 : Rank(Low Beta)
		df_full['signal_1'] = -df_full.groupby(['permno'])['beta'].shift(1).multiply(df_full['screen'])
		df_full['signal_1'] = df_full.groupby('ldate')['signal_1'].rank(axis = 1, ascending = True)

		# Signal 2 : Rank(Asset Growth)
		df_full['at12'] = df_full.groupby(['permno'])['at'].shift(12).multiply(df_full['screen12'])
		df_full['signal_2'] = df_full['at'] / df_full['at12']
		df_full['signal_2'] = df_full.groupby('ldate')['signal_2'].rank(axis = 1, ascending = True)

		# Signal 3 : Rank(profitA)
		df_full['signal_3'] = df_full.groupby('ldate')['profitA'].rank(axis = 1, ascending = True)

		# Signal is the average of the three
		df_full['signal'] = (df_full['signal_1'] + df_full['signal_2'] + df_full['signal_3']) / 3
		
		df_full = df_full.drop(columns = ['signal_1', 'signal_2', 'signal_3'])

	elif strategy_name == 'STreversal':
	
		df_full['signal'] = -df_full.groupby(['permno'])['daret'].shift(1).multiply(df_full['screen'])
	...
	df = df_full[(df_full['ldate'] <= sample_end) & (df_full['ldate'] >= sample_start)]
...
## OTHER STUFF
# df_SPY = df_ETF_raw[df_ETF_raw['ticker'] == 'SPY'].sort_values(['date', 'ym']).dropna(subset = ['retM'])
# df_SPY = df_SPY.groupby(['ym', 'ticker'])[['retM', 'ym']].transform('first').drop_duplicates().rename(columns = {'retM' : 'retMSPY'})
# df = df.merge(df_SPY, on = ['ym'], how = 'outer')
# df['reteMSPY'] = df['retMSPY'] - df['rf']